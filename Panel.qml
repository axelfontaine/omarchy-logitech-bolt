...
Panel {
  id: root
  moduleName: "axelfontaine.logitech-bolt"
  ipcTarget: "axelfontaine.logitech-bolt"

  implicitWidth: button.implicitWidth
  implicitHeight: button.implicitHeight

  readonly property string helperPath: Quickshell.env("HOME") + "/.config/omarchy/plugins/axelfontaine.logitech-bolt/bin/hidpp-receiver"
  readonly property string fixPermsPath: Quickshell.env("HOME") + "/.config/omarchy/plugins/axelfontaine.logitech-bolt/bin/fix-udev-permissions"
  ...
  // Hide the bar icon entirely when no Bolt receiver is detected, rather
  // than just dimming it.
  visible: anyReceiver
  ...
  // Discover devices, auto-pair the first one found, show the passkey the
  // receiver requests, report the result. This needs to survive the popout
  // being closed mid-flow (the user has to go type a passkey on the
  // device), so it lives in root-level state rather than anything scoped
  // to the popup.
  property string boltPhase: "idle" // idle | discovering | connecting | passkey | done
  property string boltDeviceName: ""
  // Bit 0 of this says whether the device wants a typed passkey or a
  // Left/Right click-sequence one — see Model.boltUsesTypedPasskey.
  property int boltDeviceAuthentication: 0
  property string boltPasskey: ""
  property string boltResultText: ""
  ...
  function activateCursor() {
    if (root.selectedIndex < 0) {
      if (root.boltPhase !== "idle") root.cancelBolt(); else root.startBoltDiscovery()
      return
    }
  ...
  function startBoltDiscovery() {
    if (!root.receiver) return
    root.boltPhase = "discovering"
    root.boltDeviceName = ""
    root.boltDeviceAuthentication = 0
    root.boltPasskey = ""
    root.boltResultText = ""
    boltDiscoverProc.command = [root.helperPath, "bolt-discover", root.receiver.id, "--timeout", "45"]
    boltDiscoverProc.running = true
  }

  function pairBoltDevice(device) {
    if (!device || !root.receiver) return
    if (boltDiscoverProc.running) boltDiscoverProc.running = false
    Quickshell.execDetached([root.helperPath, "pair-stop", root.receiver.id])
    root.boltPhase = "connecting"
    root.boltDeviceName = device.name || ""
    root.boltDeviceAuthentication = device.authentication || 0
    root.boltPasskey = ""
    root.boltResultText = ""
    // Bit 0 of the device's own reported authentication capability picks
    // the passkey entropy: a typed 6-digit passkey (entropy 20) or a
    // Left/Right click-sequence one (entropy 10) — this is the device's own
    // stated capability (see Model.boltUsesTypedPasskey), not something we
    // infer from its kind. Sending the typed-passkey entropy to a
    // click-confirmed device is what was asking you to "type" a code on a
    // device with no keys.
    var entropy = Model.boltUsesTypedPasskey(device.authentication) ? 20 : 10
    boltPairProc.command = [
      root.helperPath, "bolt-pair", root.receiver.id, device.address, String(device.authentication),
      "--slot", "0", "--timeout", "25", "--entropy", String(entropy)
    ]
    boltPairProc.running = true
  }

  function cancelBolt() {
    if (boltDiscoverProc.running) boltDiscoverProc.running = false
    if (boltPairProc.running) boltPairProc.running = false
    if (root.receiver) Quickshell.execDetached([root.helperPath, "pair-stop", root.receiver.id])
    root.boltPhase = "idle"
    root.boltDeviceName = ""
    root.boltDeviceAuthentication = 0
    root.boltPasskey = ""
  ...
  // unpair confirmation dialog while it's open, so Escape/arrows/Enter
  // drive its Cancel/Unpair choice instead of falling through to the
  // panel's own navigation underneath it.
  ...
  Process {
    ...
    stdout: SplitParser {
      onRead: function(line) {
        if (root.boltPhase !== "discovering") return
        var obj = null
        try { obj = JSON.parse(line) } catch (e) { return }
        if (!obj || !obj.address) return
        // Deferred: stopping boltDiscoverProc from inside its own stdout
        // callback (pairBoltDevice does that) is reentrant and unreliable.
        Qt.callLater(function() { root.pairBoltDevice(obj) })
      }
    }
    onExited: if (root.boltPhase === "discovering") root.boltPhase = "idle"
  }
  ...
  Process {
    ...
    stdout: SplitParser {
      onRead: function(line) {
        ...
        if (obj.status === "passkey") {
          root.boltPhase = "passkey"
          root.boltPasskey = obj.passkey
        } else if (obj.status === "paired") {
          root.finishBolt()
        } else if (obj.status === "failed") {
          root.boltPhase = "done"
          root.boltResultText = "Pairing failed" + (obj.error ? (": " + obj.error) : "")
        } else {
          root.boltPhase = "done"
          root.boltResultText = "Pairing timed out"
        }
      }
    }
    // If the process dies without ever printing a final status (crash,
    // killed), don't leave the UI stuck on "Connecting…" forever.
    onExited: if (root.boltPhase === "connecting") {
      root.boltPhase = "done"
      root.boltResultText = "Pairing failed"
    }
  }
  ...
  Timer {
    ...
    // discovery/pair flow is active (also kept alive through the popout
    // getting dismissed mid-flow) for snappier feedback there.
    interval: (root.opened || root.boltPhase !== "idle") ? 4000 : 15000
    repeat: true
    running: true
  ...
  BarIconButton {
    ...
    anchors.fill: parent
    bar: root.bar
    iconComponent: boltBarIconComponent
    onPressed: root.toggle()
  }

  Component {
    id: boltBarIconComponent
    Item {
      BoltIcon {
    ...
    PanelKeyCatcher {
      ...
      Column {
        ...
        Item {
          ...
          Button {
            id: pairBtn
            readonly property bool active: root.boltPhase !== "idle"
            ...
            iconText: active ? "" : "󰐕"
            text: active ? "Cancel" : ""
            tooltipText: active ? "" : "Pair new device"
            hasCursor: root.headerHasCursor
            onHovered: function(isHovered) { if (isHovered) root.setHeaderCursor() }
            onClicked: active ? root.cancelBolt() : root.startBoltDiscovery()
          }
        }
        ...
        Column {
          width: parent.width
          spacing: Style.space(6)
          visible: root.boltPhase !== "idle"

          Text {
            visible: root.boltPhase === "discovering"
            textFormat: Text.PlainText
            text: "Scanning… hold the Easy-Switch button on the new device for 5s"
          ...
          Text {
            visible: root.boltPhase === "connecting"
            textFormat: Text.PlainText
            text: "Connecting" + (root.boltDeviceName ? (" to " + root.boltDeviceName) : "") + "…"
            color: Qt.darker(root.bar.foreground, 1.5)
            font.family: root.bar.fontFamily
            font.pixelSize: Style.font.caption
          }
          ...
          Text {
            visible: root.boltPhase === "passkey"
            textFormat: Text.StyledText
            // A device with keys types the passkey directly. One without
            // (a mouse, almost always) confirms it as a Left/Right
            // click sequence instead — the passkey in 10-bit binary, MSB
            // first, "1" a right click and "0" a left click — then both
            // buttons together to confirm, the click equivalent of
            // pressing Enter. Matches Solaar's pairing UI exactly
            // (lib/solaar/ui/pair_window.py).
            text: Model.boltUsesTypedPasskey(root.boltDeviceAuthentication)
              ? ("Type <b>" + root.boltPasskey + "</b> on the new device, then press <b>Enter</b>")
              : ("Click <b>" + Model.boltClickSequence(root.boltPasskey).join(", ") + "</b> on the new device in that order, then click both buttons together to confirm")
            color: root.bar.foreground
            font.family: root.bar.fontFamily
          ...
          Item {
            visible: root.boltPhase === "done"
            width: parent.width
            implicitHeight: Math.max(resultText.implicitHeight, doneBtn.implicitHeight)
